...
#define PUSH_INS(INS) PANIC_ON_FAIL(ins_builder_append_ins(ins_builder, INS), compiler, ERROR_MEMORY)
...
const int init_compiler(compiler_t* compiler, const char* source) {
	...
	PANIC_ON_FAIL(init_ast(&compiler->ast, source), compiler, compiler->ast.last_err);
	return 1;
}
...
static const int compile_ast_proc(compiler_t* compiler, ins_builder_t* ins_builder, ast_proc_t* procedure, ast_reg_t out_reg) {
	...
	ESCAPE_ON_FAIL(compile_code_block(compiler, ins_builder, &procedure->exec_block, procedure->exec_block.register_limit, procedure, 0, 0));
...
static const int compile_ast_value_reg(compiler_t* compiler, ins_builder_t* ins_builder, ast_value_t* value, ast_reg_t* dest_reg, ast_reg_t set_regs, uint16_t temp_regs) {
	...
	{
	case AST_VALUE_SET_VAR: {
		ESCAPE_ON_FAIL(compile_ast_value(compiler, ins_builder, &value->data.set_var->set_value, value->alloced_reg, temp_regs));
	...
	case AST_VALUE_PROC_CALL: {
		ast_reg_t proc_reg;
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.proc_call->procedure, &proc_reg, TEMPREG(temp_regs), temp_regs));
		for (uint8_t i = 0; i < value->data.proc_call->argument_count; i++)
			ESCAPE_ON_FAIL(compile_ast_value(compiler, ins_builder, &value->data.proc_call->arguments[i], TEMPREG(temp_regs + i + 1), temp_regs + i + 2));
...
static const int compile_ast_value(compiler_t* compiler, ins_builder_t* ins_builder, ast_value_t* value, ast_reg_t out_reg, uint16_t temp_regs) {
	...
	case AST_VALUE_PROC_CALL: {
		ast_reg_t reg;
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, value, &reg, out_reg, temp_regs));
		PUSH_INS(INS2(OP_CODE_MOVE, out_reg, reg));
		break; 
	}
	case AST_VALUE_ALLOC_ARRAY: {
		ast_reg_t len_reg;
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.alloc_array->size, &len_reg, out_reg, temp_regs));
	...
	case AST_VALUE_ARRAY_LITERAL: {
		...
		for (uint_fast32_t i = 0; i < value->data.array_literal.element_count; i++) {
			ast_reg_t elem_reg;
			ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.array_literal.elements[i], &elem_reg, TEMPREG(temp_regs), temp_regs + 1));
			PUSH_INS(INS3(OP_CODE_STORE_HEAP_I, out_reg, GLOBREG(i), elem_reg));
		}
		break;
	}
	case AST_VALUE_PROC:
		ESCAPE_ON_FAIL(compile_ast_proc(compiler, ins_builder, value->data.procedure, out_reg));
		break;
	case AST_VALUE_GET_INDEX: {
		ast_reg_t array_reg;
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.get_index->array, &array_reg, out_reg, temp_regs));
		if (value->data.get_index->index.value_type == AST_VALUE_LONG)
			...
			ast_reg_t index_reg;
			ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.get_index->index, &index_reg, TEMPREG(temp_regs), temp_regs + 1));
	...
	case AST_VALUE_SET_INDEX: {
		...
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.set_index->array, &array_reg, TEMPREG(temp_regs), temp_regs + 1));
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.set_index->value, &value_reg, out_reg, temp_regs + 1));
		if (value->data.set_index->index.value_type == AST_VALUE_LONG)
			...
			ast_reg_t index_reg;
			ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.set_index->index, &index_reg, TEMPREG(temp_regs + 1), temp_regs + 2));
	...
	case AST_VALUE_BINARY_OP: {
		...
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.binary_op->lhs, &lhs_reg, out_reg, temp_regs));
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.binary_op->rhs, &rhs_reg, TEMPREG(temp_regs), temp_regs + 1));
	...
	case AST_VALUE_UNARY_OP: {
		ast_reg_t op_reg;
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &value->data.unary_op->operand, &op_reg, out_reg, temp_regs));
...
static const int compile_conditional(compiler_t* compiler, ins_builder_t* ins_builder, ast_cond_t* conditional, uint16_t temp_regs, ast_proc_t* procedure, uint16_t break_jump, uint16_t continue_jump) {
	if (conditional->has_cond_val) {
		...
		ast_reg_t cond_reg;
		ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &conditional->cond_val, &cond_reg, TEMPREG(temp_regs), temp_regs + 1));
		...
		if (conditional->next_if_false) {
			ESCAPE_ON_FAIL(compile_code_block(compiler, ins_builder, &conditional->exec_block, temp_regs + 1, procedure, break_jump, continue_jump));
			...
			ESCAPE_ON_FAIL(compile_conditional(compiler, ins_builder, conditional->next_if_false, temp_regs, procedure, break_jump, continue_jump));
			ins_builder->instructions[cond_begin_ip].a = ins_builder->instruction_count;
		}
		else {
			if (conditional->next_if_true) {
				ESCAPE_ON_FAIL(compile_code_block(compiler, ins_builder, &conditional->exec_block, temp_regs + 1, procedure, body_jump_ip, this_begin))
...
static const int compile_code_block(compiler_t* compiler, ins_builder_t* ins_builder, ast_code_block_t* code_block, uint16_t temp_regs, ast_proc_t* procedure, uint16_t break_jump, uint16_t continue_jump) {
	for (uint_fast32_t i = 0; i < code_block->instruction_count; i++) {
		...
		{
		case AST_TOP_LEVEL_DECL_VAR:
			ESCAPE_ON_FAIL(compile_ast_value(compiler, ins_builder, &code_block->instructions[i].data.var_decl.set_value, code_block->instructions[i].data.var_decl.var_info.alloced_reg, temp_regs));
			break;
		case AST_TOP_LEVEL_VALUE:
			ESCAPE_ON_FAIL(compile_ast_value(compiler, ins_builder, &code_block->instructions[i].data.value, TEMPREG(temp_regs), temp_regs + 1));
			break;
		case AST_TOP_LEVEL_COND:
			ESCAPE_ON_FAIL(compile_conditional(compiler, ins_builder, code_block->instructions[i].data.conditional, temp_regs, procedure, break_jump, continue_jump));
			break;
		case AST_TOP_LEVEL_RETURN_VALUE: {
			ESCAPE_ON_FAIL(compile_ast_value(compiler, ins_builder, &code_block->instructions[i].data.value, TEMPREG(0), temp_regs));
		...
		case AST_TOP_LEVEL_BREAK:
			PANIC_ON_FAIL(break_jump, compiler, ERROR_CANNOT_BREAK);
			PUSH_INS(INS1(OP_CODE_JUMP, GLOBREG(break_jump)));
			break;
		case AST_TOP_LEVEL_CONTINUE:
			PANIC_ON_FAIL(continue_jump, compiler, ERROR_CANNOT_CONTINUE);
			PUSH_INS(INS1(OP_CODE_JUMP, GLOBREG(continue_jump)));
			break;
		case AST_TOP_LEVEL_FOREIGN: {
			ast_reg_t id_reg;
			ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &code_block->instructions[i].data.foreign.id_t, &id_reg, TEMPREG(temp_regs), temp_regs + 1));

			if (code_block->instructions[i].data.foreign.has_input) {
				ast_reg_t op_reg;
				ESCAPE_ON_FAIL(compile_ast_value_reg(compiler, ins_builder, &code_block->instructions[i].data.foreign.input, &op_reg, TEMPREG(temp_regs + 1), temp_regs + 2));
...
const int compile(compiler_t* compiler, machine_t* machine, machine_ins_t** output_ins, uint16_t* output_count) {
	PANIC_ON_FAIL(init_machine(machine, UINT16_MAX, 1000, 1000), compiler, ERROR_MEMORY);
	...
	PANIC_ON_FAIL(init_ins_builder(&ins_builder), compiler, ERROR_MEMORY);
	...
	ESCAPE_ON_FAIL(compile_code_block(compiler, &ins_builder, &compiler->ast.exec_block, compiler->ast.exec_block.register_limit, NULL, 0, 0));

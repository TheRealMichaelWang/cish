...
int machine_execute(machine_t* machine, machine_ins_t* instructions) {
	...
	while (1) {
		...
		case OP_CODE_LOAD_HEAP_I: {
			machine_reg_t array_register = machine->stack[AREG];
			if (!array_register.heap_alloc->init_stat[ip->b])
				PANIC(machine, ERROR_READ_UNINIT);
			machine->stack[CREG] = array_register.heap_alloc->registers[ip->b];
			break;
		}
		...
		case OP_CODE_STORE_HEAP_I: {
			machine_reg_t array_register = machine->stack[AREG];
			array_register.heap_alloc->registers[ip->b] = machine->stack[CREG];
			array_register.heap_alloc->init_stat[ip->b] = 1;
			break;
		}
